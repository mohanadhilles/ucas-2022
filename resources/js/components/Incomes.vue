...
<div class="main_content inner">
    ...
    <div class="report expense-report">
        <div class="div-title" style = "background-color: #638E43;">
            <span class="income-icon">الايردات اليومية</span>
            <span><router-link to="#" onclick="openFormIncome()"><i class="fa fa-plus"></i> اضافة</router-link></span>

            
            <div class="tables_inc">
                    <input type="date" v-model="SearchDay" @change="filteredpro()"  style="background-color: #638e43;">
                <div>
                    <table style="margin-top: 8px;" class="table sms-table">
                        <thead>
                            <tr>
                                <th style="width:15px;border-right:none;">#</th>
                                <th style="width:129px;">الصنف</th>
                                <th style="width:54px;">الكمية</th>
                                <th style="width:81px;">سعر الوحدة</th>
                                <th style="width:69px;">المجموع</th>
                                <th style="width:170px;">ملاحظات</th>
                                <th style="width:86px;">التاريخ/الوقت</th>
                                <th style="width:110px;">العمليات</th>
                            </tr>
                        </thead>
                    </table>
                </div>
                ...
                <div>
                    <table class="table sms-table">
                        <tbody style="height: 265px;">
                            <tr v-for="(Data , index) in dataIncome" :key="index">
                                <td style="width:32px;border-right:none;line-break: anywhere;">{{ index+1 }}</td>
                                <td style="width:466px;line-break: anywhere;">{{ Data.service }}</td>
                                <td style="width:9%;line-break: anywhere;">{{ Data.quantity }}</td>
                                <td style="width:12.1%;line-break: anywhere;">{{ Data.price }}</td>
                                <td style="width:11%;line-break: anywhere;">{{Data.price*Data.quantity}}</td>
                                <td style="width:21.4%;line-break: anywhere;">{{ Data.note }}</td>
                                <td style="width: 341px;font-size: 13px;line-break: anywhere;font-family: none;">{{ Data.created_at }}</td>
                                <td style="width:377px;text-align: center;letter-spacing: 11px;line-break: anywhere;">
                    ...
                    <table class="table sms-table">
                        <tbody>
                            <tr>
                                <td style="background-color: rgb(99, 142, 67);width:300px;color: #ffffff;border-left: 1px solid #ffffff;" colspan="3">اجمالي الإيرادات :</td>
                                <td style="background-color: rgb(99, 142, 67);width:388px;color: #ffffff;">{{mjmoa}} ₪</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
export default {
    ...
    methods: {
        ...
        filteredpro() {
            ...
            axios
            .get(`api/Search/${this.SearchDay}?token=` + token)
            .then((res) => {
            this.dataIncome = res.data;
            console.log(res.data)
            })
